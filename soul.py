...
BOT_TOKEN = "8394300353:AAH05W3GqEuyKnGaJkjTsT8722YFn3XtmNE"
YML_FILE_PATH = ".github/workflows/main.yml"
BINARY_FILE_NAME = "soul"
BINARY_STORAGE_PATH = "stored_binary.bin"
ADMIN_IDS = [6302867468, 5090526695]
...
async def show_user_management(update: Update, user_id):
    if not (is_owner(user_id) or is_admin(user_id)):
        await update.message.reply_text("⚠️ **ACCESS DENIED DM @ZEXYxERA TO GET ACCESS**")
        return
...
async def set_cooldown_start(update: Update, user_id):
    if not (is_owner(user_id) or is_admin(user_id)):
        await update.message.reply_text("⚠️ **ACCESS DENIED DM @ZEXYxERA TO GET ACCES**")
        return
